...
| 2026-04-21 | sandbox runtime | Windows 后端从 noop smoke 推进到真实本机策略实现，并暴露当时的启动兼容性风险。 | 新增 Windows local-user 后端，支持 ACL 文件读写策略、Job Object 进程树清理、offline/open 网络模式；UTM Windows arm64 复验发现备用凭据启动部分系统程序会返回 `0xC0000142`，该问题已在 2026-04-22 的后续迭代中解决。 |
...
| 2026-04-22 | sandbox runtime | 三端 SDK 调用链路具备可重复 E2E，Windows setup/allowlist 进入闭环。 | 新增 SDK `Setup`、CLI `setup windows`、helper binary resolution、Windows host-proxy + per-user firewall allowlist、`tests/e2e` 和 `scripts/e2e-sdk.{sh,ps1}`；macOS/Linux/Windows 均验证文件隔离、offline、allowlist 允许/拒绝和直连绕过阻断。 |
